...
# Switch to production or testing
TESTING_MODE = False
# Initial-mail pause: 60 seconds (1 minute)
INITIAL_DELAY = 60  # seconds
# Follow-up delays
FIRST_FOLLOWUP_DELAY = 86400  # 86400 seconds = 1 day
SECOND_FOLLOWUP_DELAY = 86400  # send second follow-up 1 day after the first
...
def send_initial_email(row):
    ...
    try:
        with smtplib.SMTP(sender['smtp_server'], sender['smtp_port'], timeout=10) as server:
            ...
            print(f"Initial email sent to {to_addr}")
...
def followup_scheduler(to_addr, msg_id, name, company, sender, subj):
    ...
    delay1 = 10 if TESTING_MODE else FIRST_FOLLOWUP_DELAY
    time.sleep(delay1)
    ...
    delay2 = 10 if TESTING_MODE else SECOND_FOLLOWUP_DELAY
    time.sleep(delay2)
    if not check_reply(to_addr):
...
def send_emails(xlsx_path):
    ...
    for _, row in df.iterrows():
        ...
        # Launch follow-ups in a separate thread so initial-loop isn't blocked
        threading.Thread(
        ...
        # Pause before next initial email
