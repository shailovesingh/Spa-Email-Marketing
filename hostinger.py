...
def spin_email_template(person_name, company, is_followup=False, followup_number=None):
    ...
    # Plain‑text bullet points (sentence2_text):
    ...
    # HTML bullet points (sentence2_html):
...
def send_emails(xlsx_path):
    ...
    for _, row in df.iterrows():
        ...
        # Either run follow‑ups immediately in the same thread (blocking):
        followup_scheduler(email, msg_id, name, company, sender, subj)

        # Pause for 60 seconds between recipients
        time.sleep(60)
